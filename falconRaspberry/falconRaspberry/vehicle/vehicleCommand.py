...
class VehicleCommand(object):
    ...
    def __obstacleUp(self):
        ...
        if self.actionRunning != None and self.actionRunning.isAdvance() and not self.actionRunning.isStop:
            ...
            self.actionRunning = type(action)(action.puissance)
            del(action)
        self.canAdvance = False
        ...
        if isinstance(action,ActionVehicle) and ((self.canAdvance and action.isAdvance()) or (self.canReverse and action.isReverse()) or (not action.isAdvance() and not action.isReverse()) ) : 
            if type(action) == type(self.actionRunning):
                if not self.actionRunning.setPuissance(puissance):
                    self.actionRunning.stop()
                    action.puissanceActu = self.actionRunning.puissanceActu if self.actionRunning.puissanceActu != action.puissance else action.puissance - 1
